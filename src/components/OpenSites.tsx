import React from 'react';
  ...
  const OpenSites: React.FC = () => {
    const openSites = PROPERTIES.filter(property => property.type === 'Open Site');
    ...
    return (
      <section className="py-12 bg-gray-100">
        <div className="container mx-auto px-4">
          ...
          {openSites.length === 0 ? (
            <p className="text-center text-gray-600">No Open Sites available at the moment.</p>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {openSites.map(property => (
                <div key={property.id} className="bg-white p-6 rounded-lg shadow-md">
                  ...
                  <p className="text-gray-700 mb-2">
                    <strong>Price:</strong> ₹{property.price.toLocaleString()} {property.areaUnit === 'sq.yrds' ? 'per sq yd' : ''}
                  </p>
                  <p className="text-gray-700 mb-2">
